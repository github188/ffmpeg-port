...
CKDVDecoderImpl * CKDVDecoderImpl::GetDecoder( CBaseCodec::ECodecId eCodecId, LPCTSTR strName /* = NULL */ )
{
	...
	if ( NULL == pDec )
	{
		mcu::tlog << _T( "CKDVDecoderImpl::GetDecoder new CKDVDecoderImpl Fail!" ) << endl;
		return NULL;
	}
	...
	if ( !bResult )
	{
		mcu::tlog << _T( "keda decoder Create Fail! codecid: " ) << eCodecId << endl;
	}
...
BOOL CKDVDecoderImpl::Create( CDecoder::ECodecId eCodec )
{
	...
	default:
		mcu::tlog << _T( "unkown kdv codec Id " ) << eCodec << endl;
		_ASSERT( FALSE );
		return FALSE;
	    break;
	}
	...
	if ( NULL == m_pDecParam )
	{
		mcu::tlog << _T( "m_pDecParam = (TVideoUnitDecParam*)mu_mallocz mu_mallocz 失败！" ) << endl;
		return FALSE;
	}
	...
	if( NULL == m_pvHandle )
	{
		...
		{
			mcu::tlog << _T("VideoUnitDecoderOpen failure!") << endl;
		...
		{
			mcu::tlog << _T( "Craete H264 Decoder success! VideoUnitDecoderOpen" ) << endl;
		}
	...
	{
		mcu::tlog << _T( "VideoUnitDecoderSetParam success! codec change to: " ) << m_pDecParam->tDecParam.u32DecType <<endl;
	}
	...
	if ( NULL == m_pPicDataBuf )
	{
		...
		if ( NULL == m_pPicDataBuf )
		{
			mcu::tlog << "m_pPicDataBuf = ( mu_uint8 * )mu_mallocz( lPicBufLen ) 申请内存失败！" << endl;
			return FALSE;
		}
	}
							 ...
							 CBaseCodec::TVideoFrameInfo *pFrameInfo/* = NULL*/)
{
	...
	////mcu::tlog << _T( "Decode pkt timespan: " ) << nSpan << endl;
...
BOOL CKDVDecoderImpl::CapturePic( LPCTSTR strPicPath, CEncoder *pEnc )
{
	...
	if ( bResult )
	{
		...
		{
			mcu::tlog << "Capture pic open file fail! file: " << strPicPath << endl;
			bResult = FALSE;
		}		
	}	
	return bResult;
