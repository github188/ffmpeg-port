    ...
    if (tag == MP4DecConfigDescrTag) {
        ...
        if (tag == MP4DecSpecificDescrTag) {
            ...
            if (st->codec->codec_id == CODEC_ID_AAC) {
				av_log( c->fc, AV_LOG_ERROR, "thinkingl: don't support audio!!!" );
...
static int mov_read_smi(MOVContext *c, ByteIOContext *pb, MOVAtom atom)
{
    ...
    get_buffer(pb, (char*)(st->codec->extradata) + 0x5a, atom.size);
...
static int mov_read_wave(MOVContext *c, ByteIOContext *pb, MOVAtom atom)
{
	...
	av_log(c->fc, AV_LOG_ERROR, "thinkingl: 3gp don't support audio!!!" );
...
{
	av_log(NULL, AV_LOG_ERROR, "thinkingl: 3gp don't support audio!!!" );
...
static int mov_read_stsd(MOVContext *c, ByteIOContext *pb, MOVAtom atom)
{
 ...
 //           MOVAtom fake_atom = { .size = size - (url_ftell(pb) - start_pos) };
			MOVAtom fake_atom;
			memset( &fake_atom, 0, sizeof(fake_atom) );
			fake_atom.size = size - (url_ftell(pb) - start_pos);
    ...
    case CODEC_ID_ALAC:
        if (st->codec->extradata_size == 36) {
            st->codec->frame_size = AV_RB32( ( char* )( st->codec->extradata )+12);
            st->codec->channels   = AV_RB8 ( (char *)st->codec->extradata+21);
        }
        break;
    default:
        break;
    ...
    if (sc->dref_id-1 < sc->drefs_count && sc->drefs[sc->dref_id-1].path) {
        if (url_fopen(&sc->pb, sc->drefs[sc->dref_id-1].path, URL_RDONLY) < 0)
            av_log(c->fc, AV_LOG_ERROR, "stream %d, error opening file %s: %s\n",
                   st->index, sc->drefs[sc->dref_id-1].path, 0 /*strerror(errno)*/);
    } else
        sc->pb = c->fc->pb;
...
static int mov_read_packet(AVFormatContext *s, AVPacket *pkt)
{
 ...
 retry:
    ...
    if (!sample) {
		...
		tmpAtom.size = INT64_MAX;
		tmpAtom.offset = 0;
		tmpAtom.type = 0;

        if (!url_is_streamed(s->pb) ||
            mov_read_default(mov, s->pb, tmpAtom /**(MOVAtom){ 0, 0, INT64_MAX }*/ ) < 0 ||
            url_feof(s->pb))
            return -1;
