...
//    ff_isom_write_avcc(pb, track->vosData, track->vosLen);
	...
	memcpy( &pps_size, track->vosData + nCursor, sizeof( int ) );
	nCursor += sizeof( int );

	pps = track->vosData + nCursor;
	nCursor += pps_size;

	memcpy( &sps_size, track->vosData + nCursor, sizeof( int ) );
	nCursor += sizeof( int );

	sps = track->vosData + nCursor;
	nCursor += sps_size;
...
static int mov_find_codec_tag(AVFormatContext *s, MOVTrack *track)
{
    ...
    if (track->mode == MODE_MP4 || track->mode == MODE_PSP) {
		av_log(s, AV_LOG_ERROR, "thinkingl: 3gp don't support mp4 or psp mode!!!" );
...
//        else if (track->enc->codec_type == CODEC_TYPE_AUDIO) tag = MKTAG('m','p','4','a');
		...
		av_log(s, AV_LOG_ERROR, "thinkingl: 3gp don't support ipod mode!!!" );
...
//	av_log( 0, AV_LOG_ERROR, "before mov_write_stbl_tag\n" );
...
//	av_log( 0, AV_LOG_ERROR, "before stts \n" );
...
//	av_log( 0, AV_LOG_ERROR, "before stsc \n" );

    mov_write_stsc_tag(pb, track);
    mov_write_stsz_tag(pb, track);
    mov_write_stco_tag(pb, track);

//	av_log( 0, AV_LOG_ERROR, "end mov_write_stbl_tag" );
    return updateSize(pb, pos);
}
...
static int mov_write_hdlr_tag(ByteIOContext *pb, MOVTrack *track)
{
	...
	// thinkingl 原为 put_buffer,改为put_string.
	put_string( pb, descr );
...
{
	// thinkingl : 这个free标签没有用，标准中没有，且会造成tcpmp播放器错误！
...
static int mov_write_header(AVFormatContext *s)
{
    ...
    for(i=0; i<s->nb_streams; i++){
        ...
        if(st->codec->codec_type == CODEC_TYPE_VIDEO){
			...
			av_log(s, AV_LOG_ERROR, "thinkingl: 3gp don't support audio!!!" );
			...
			av_log(s, AV_LOG_ERROR, "thinkingl: 3gp don't support subtitle!!!" );
...
static int mov_write_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    if ((/*enc->codec_id == CODEC_ID_DNXHD ||*/
                enc->codec_id == CODEC_ID_AC3) && !trk->vosLen) {
					av_log(s, AV_LOG_ERROR, "thinkingl: 3gp don't support ac3!!!!" );
        ///* copy frame to create needed atoms */
        //trk->vosLen = size;
        //trk->vosData = av_malloc(size);
        //if (!trk->vosData)
        //    return AVERROR( 99999 );	// lizhixing!!!!
...
//	av_log( s, AV_LOG_ERROR, "before mov_write_moov_tag\n" );
...
const AVCodecTag* gp_codec_ar[2]={codec_3gp_tags, 0};
...
AVOutputFormat tgp_muxer = {
    ...
    gp_codec_ar,	//(const AVCodecTag* const []){codec_3gp_tags, 0}, //codec_tag
	CODEC_ID_NONE ,	// subtitle_codec
	NULL,
	NULL,
