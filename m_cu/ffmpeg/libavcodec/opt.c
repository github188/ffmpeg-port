...
static AVOption *av_set_number(void *obj, const char *name, double num, int den, int64_t intnum){
    ...
    case FF_OPT_TYPE_RATIONAL:
		tempThinkingl.den = den;
		tempThinkingl.num = num*intnum;
//		if((int)num == num) *(AVRational*)dst= (AVRational){num*intnum, den};
		if((int)num == num) *(AVRational*)dst= tempThinkingl;
...
//		return (AVRational){intnum, den};
		tempThinkingl.den = den;
		tempThinkingl.num = intnum;
		return tempThinkingl;	// thinkingl!
	}
    else
