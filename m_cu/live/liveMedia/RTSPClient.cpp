    ...
    fServerIsMicrosoft(False)
{
  ...
  char const* const libName = "KEDACOM thinkingl Streaming Media v";
  char const* const libVersionStr = LIVEMEDIA_LIBRARY_VERSION_STRING;
...
Boolean RTSPClient::setupMediaSubsession(MediaSubsession& subsession,
  ...
  do {
	...
	// 增加response code 的传递。by thinkingl @ 2009.09.28
    Boolean bGetRsp = getResponse("SETUP", bytesRead, responseCode, firstLine, nextLineStart);
	if ( pResponseCode )
	{
...
*/
 ...
 //       if (errno != WSAEINPROGRESS) {
		  int nError = envir().getErrno();
		  int nree = WSAGetLastError();
