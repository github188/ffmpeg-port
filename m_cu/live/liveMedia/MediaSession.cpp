...
Boolean MediaSubsession::parseSDPAttribute_bandwidth(char const* sdpLine)
{
	// Check for a "b=AS:<bandwidth>" or "b=CT:<bandwidth>"

	Boolean parseSuccess = False;
	
	int nBandWidth;
	if (sscanf(sdpLine, "b=AS: %d", &nBandWidth) == 1 || sscanf(sdpLine, "b=AS:%d", &nBandWidth) == 1) {
		parseSuccess = True;
		m_nBandWidth = nBandWidth;
	} else if (sscanf(sdpLine, "b=CT: %d", &nBandWidth) == 1 || sscanf(sdpLine, "b=CT:%d", &nBandWidth) == 1) {
		parseSuccess = True;
		m_nBandWidth = nBandWidth;
	}
